...
OUT_FILE = Path("output") / "table_bu_by_fr1.xlsx"
...
def main():
    ...
    if len(conflicts) > 0:
        print("🔴 RESULT: KO (SKU appears in multiple BU within same FR1 in the period)")
    ...
    if g2[COL_BU].isna().any():
        ...
        print("🔴 RESULT: KO (Missing BU mapping for some (FR1, SKU))")
