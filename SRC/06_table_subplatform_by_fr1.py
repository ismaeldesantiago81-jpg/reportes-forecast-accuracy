...
OUT_FILE = Path("output") / "table_subplatform_by_fr1.xlsx"
...
def main():
    ...
    if not EXCEL_FILE.exists():
        print(f"❌ Missing raw file: {EXCEL_FILE.as_posix()}")
        return
    ...
    if len(conflicts) > 0:
        print("🔴 RESULT: KO (SKU appears in multiple Subplataformas within same FR1 in the period)")
    ...
    if g2[COL_SUBPLAT].isna().any():
        ...
        print("🔴 RESULT: KO (Missing Subplataforma mapping for some (FR1, SKU))")
